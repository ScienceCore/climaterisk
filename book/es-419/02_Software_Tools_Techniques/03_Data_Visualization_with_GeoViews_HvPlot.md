...
Las principales herramientas que utilizaremos para el trazado provienen de la familia [Holoviz](https://holoviz.org/) de bibliotecas Python, principalmente [GeoViews](https://geoviews.org/) and [hvPlot](https://hvplot.holoviz.org/). Estas están construidas en gran parte sobre [HoloViews](https://holoviews.org/) y soportan múltiples backends para la representación de gráficos (notably [Bokeh](http://bokeh.pydata.org/) para visualización interactiva y [Matplotlib](http://matplotlib.org/) para gráficos estáticos con calidad de publicación).
...
<center><img src="https://geoviews.org/_static/logo_horizontal.png">
</center>

De la documentación de [GeoViews documentation](https://geoviews.org/index.html):

> GeoViews es una biblioteca de [Python](http://python.org/) que facilita la exploración y visualización de conjuntos de datos geográficos, meteorológicos y oceanográficos, como los que se utilizan en la investigación meteorológica, climática y de teledetección.
>
> GeoViews se basa en la biblioteca [HoloViews](http://holoviews.org/) y permite crear visualizaciones flexibles de datos multidimensionales. GeoViews agrega una familia de tipos de trazados geográficos basados en la biblioteca [Cartopy](http://scitools.org.uk/cartopy), trazados con los paquetes [Matplotlib](http://matplotlib.org/) o [Bokeh](http://bokeh.pydata.org/). Con GeoViews, puedes trabajar de forma fácil y natural con grandes conjuntos de datos geográficos multidimensionales, visualizando al instante cualquier subconjunto o combinación de ellos. Al mismo tiempo, podrás acceder siempre a los datos brutos subyacentes a cualquier gráfico.
...
Un _mapa base_ o _capa de mosaico_ es útil cuando se muestran datos vectoriales o ráster porque nos permite superponer los datos geoespaciales relevantes sobre un mapa gepgráfico conocido como fondo. La principal funcionalidad que utilizaremos es `gv.tile_sources`. Podemos utilizar el método `opts` para especificar parámetros de configuración adicionales. A continuación, utilizaremos _Open Street Map (OSM)_ Web Map Tile Service para crear el objeto mapa base `basemap`. Cuando mostramos la representación de este objeto en la celda del cuaderno, el menú Bokeh que está a la derecha permite la exploración interactiva.
...
Podemos probar la función anterior utilizando las coordenadas de longitud y latitud de Marrakesh, Marruecos.
...
La utilidad `geoviews.Rectangles` acepta una lista de cuadros delimitadores (cada uno descrito por una tupla de la forma `(x_min, y_min, x_max, y_max)`) para el trazado. También podemos utilizar `geoviews.opts` para adaptar el rectángulo a nuestras necesidades.
...
Podemos trazar un punto para Marrakech como antes utilizando `geoviews.Points` (personalizado utilizando `geoviews.opts`).
...
<center><img src="https://hvplot.holoviz.org/_images/diagram.svg">
</center>
