...
Este cuaderno contiene las instrucciones para iniciar sesión en la infraestructura en la nube ([JupyterHub](https://jupyter.org/hub)) proporcionada por [2i2c](https://2i2c.org) para este tutorial.
...
1. **Navega hasta el 2i2c Hub:** Dirige tu navegador web a [this link](https://climaterisk.opensci.2i2c.cloud) (este enlace).
...
![2i2c\_login](../assets/2i2c_login.png)
...
![start\_server2](../assets/start_server_2i2c.png)
...
![work\_environment\_jupyter\_lab](../assets/work_environment_jupyter_lab.png)
...
Para acceder a los productos de NASA EarthData desde programas de Python o cuadernos de Jupyter, es necesario que guardes tus credenciales de NASA EarthData en un archivo especial llamado .netrc en tu directorio de inicio.

- Puedes crear este archivo ejecutando el script make_netrc.py en una terminal:
  ...
  - El script pedirá tu nombre de usuario y contraseña de NASA EarthData, así que mantente atento si lo ejecutas desde un cuaderno de Jupyter.
...
- De manera alternativa, puedes crear un archivo que se llame .netrc en tu carpeta de inicio (es decir, ~/.netrc) con el siguiente contenido:
  ...
  ```
  Por supuesto, debes reemplazar `USERNAME` y `PASSWORD` en el archivo .netrc con los datos reales de tu cuenta NASA EarthData. Una vez que hayas guardado el archivo .netrc con tus credenciales correctas, es una buena práctica restringir el acceso al archivo:
  ```bash
  $ chmod 600 ~/.netrc
  ```

## 3. Verificación del acceso a los productos de NASA EarthData
...
Una vez más, puedes ejecutarlo directamente desde este cuaderno utilizando la celda de Python que se muestra a continuación:
...
Si funcionó sin problemas, ¡ya está todo listo! ¡Ahora tienes todo lo que necesitas para explorar los datos de observación de la Tierra por parte de la NASA mediante el portal EarthData!
