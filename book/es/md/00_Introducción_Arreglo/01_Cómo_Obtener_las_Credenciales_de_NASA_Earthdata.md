---
jupyter:
  jupytext:
    text_representation:
      extension: .md
      format_name: markdown
      format_version: '1.3'
      jupytext_version: 1.17.1
  kernelspec:
    display_name: Python 3 (ipykernel)
    language: python
    name: python3
---

# Cómo obtener las credenciales de NASA Earthdata

<!-- #region jupyter={"source_hidden": true} -->
En este cuaderno se describe el proceso para obtener las credenciales de NASA Earthdata.

**Puedes completar este paso antes del día que se dicte el tutorial.**
<!-- #endregion -->

<!-- #region jupyter={"source_hidden": false} -->
---
<!-- #endregion -->

## Breve introducción

<!-- #region jupyter={"source_hidden": true} -->
El programa [Sistemas de datos científicos de la Tierra (ESDS, por sus siglas en inglés) de la NASA](https://www.earthdata.nasa.gov/) supervisa el ciclo de vida de los datos de ciencias de la Tierra de todas sus misiones de observación terrestre, desde su adquisición hasta su procesamiento y distribución.

Para fines de esta guía, el sitio web NASA Earthdata es el punto de entrada que permite el acceso completo, gratuito y abierto a las colecciones de datos de ciencias de la Tierra de la NASA, con el fin de acelerar el progreso científico en beneficio de la sociedad. Para acceder a los datos mediante este portal, quienes deseen usarlos deben definir primero sus credenciales de acceso. Para crear una cuenta en EarthData, sigue estos pasos:
<!-- #endregion -->

<!-- #region jupyter={"source_hidden": false} -->
---
<!-- #endregion -->

## Creación las Credenciales de NASA Earthdata

<!-- #region jupyter={"source_hidden": true} -->
Si ha registrado previamente las credenciales de NASA Earthdata *proceda al siguiente cuaderno* (es decir, no es necesario registrar un nuevo perfil de Earthdata de la NASA).
<!-- #endregion -->

<!-- #region jupyter={"source_hidden": true} -->
Si no te has registrado previamente, para crear una cuenta de EarthData, sigue estos pasos:

+ Ingresa al sitio web de EarthData de la NASA: [`https://urs.earthdata.nasa.gov/`](https://urs.earthdata.nasa.gov/).
+ Deberías ver algo parecido a esto:
   ![earthdata_login](../../../assets/img/earthdata_login.png)
+ Haga clic en "*Register for a profile*".
    + Una vez que se cargue la nueva página, verá algo como esto:
       ![earthdata_profile](../../../assets/img/earthdata_profile2.png)
    + Elija un nombre de usuario y una contraseña de NASA Earthdata. **Necesitará estas credenciales más adelante**; ¡Asegúrate de grabarlos de forma segura.
    + También deberá proporcionar otra información (por ejemplo, correo electrónico, país, afiliación, etc.) para completar el registro.
    + Una vez que haya completado todos los campos obligatorios, haga clic en "*Register for Earthdata Login*".
<!-- #endregion -->

<!-- #region jupyter={"source_hidden": false} -->
---
<!-- #endregion -->
